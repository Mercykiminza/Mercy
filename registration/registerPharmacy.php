...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $db = new Database();
    $query = "SELECT * FROM pharmacy WHERE emailAddress = ? OR phoneNumber = ?";
    ...
    // If no errors, insert the new pharmacy
    if (empty($errors)) {
        $insertQuery = "INSERT INTO pharmacy (name, location, emailAddress, phoneNumber) 
                        VALUES (?, ?, ?, ?)";
        $insertValues = array($name, $location, $email, $phoneNumber);
        $db->insertData($insertQuery, $insertValues);

        header('Location: registerPharmacy.php');
        exit();
    }
}
