...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $db = new Database();
    $query = "SELECT * FROM pharmaceutical WHERE emailAddress = ? OR phoneNumber = ?";
    ...
    // If no errors, insert the new pharmaceutical
    if (empty($errors)) {
        $insertQuery = "INSERT INTO pharmaceutical (name, location, emailAddress, phoneNumber) 
                        VALUES (?, ?, ?, ?)";
        $insertValues = array($name, $location, $email, $phoneNumber);
        $db->insertData($insertQuery, $insertValues);

        header('Location: registerPharmaceutical.php');
        exit();
    }
}
?>
